...
class BatchLoader(object):
	...
	def __init__(self, params, result):
		...
		self.multilabels = np.where(np.random.rand(len(self.data), self.n_labels)>=0.5, 1, -1)
		# get list of image indexes.
		self._cur = 0  # current image
		self.n_data = len(self.data) #5000 if self.is_train else 10000
	...
	def update_data(self, traindata, H=None):
		self.data = traindata
		if H is not None:
